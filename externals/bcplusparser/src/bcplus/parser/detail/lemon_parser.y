...
%nonassoc FLOAT.         // Any positive float.
...
statement(stmt) ::= rate_decl(r).					{ stmt = r; }
statement(stmt) ::= alwayst_stmt(a).					{ stmt = a; }
...
constant(c) ::= MODE(id).												{ BASE_ELEM_DEF(c, id, NULL, NULL, NULL, Symbol::Type::CONSTANT, Constant, ConstantSymbol); }
...
term(t_new) ::= SIN(a) PAREN_L term(t) PAREN_R.							{ UNARY_ARITH(t_new, a, t, UnaryTerm::Operator::SIN); }
term(t_new) ::= COS(a) PAREN_L term(t) PAREN_R.							{ UNARY_ARITH(t_new, a, t, UnaryTerm::Operator::COS); }
term(t_new) ::= TAN(a) PAREN_L term(t) PAREN_R.							{ UNARY_ARITH(t_new, a, t, UnaryTerm::Operator::TAN); }
...
term_strong(t) ::= FLOAT(f).									{ BASIC_TERM(t, f);	}
...
term_strong(t_new) ::= SIN(a) PAREN_L term(t) PAREN_R.							{ UNARY_ARITH(t_new, a, t, UnaryTerm::Operator::SIN); }
term_strong(t_new) ::= COS(a) PAREN_L term(t) PAREN_R.							{ UNARY_ARITH(t_new, a, t, UnaryTerm::Operator::COS); }
term_strong(t_new) ::= TAN(a) PAREN_L term(t) PAREN_R.							{ UNARY_ARITH(t_new, a, t, UnaryTerm::Operator::TAN); }
...
term_no_const_strong(t) ::= FLOAT(f).												{ BASIC_TERM(t, f);	}
...
term_no_const(t) ::= FLOAT(f).								{ BASIC_TERM(t, f);	}
...
term_temporal(t) ::= FLOAT(f).															{ BASIC_TERM(t, f);	}
...
term_temporal_strong(t) ::= FLOAT(f).														{ BASIC_TERM(t, f);	}
...
sort_id(s) ::= IDENTIFIER(id).
	{
		...
		if (!s) {
			parser->_parse_error("\"" + Symbol::genName(*id->str(),0) + "\" is not a declared sort test.", &id->beginLoc());
			YYERROR;
		}
		delete id;
	}
...
constant_bnd(bnd) ::= constant_dcl_lst(names) DBL_COLON constant_dcl_type(type) PAREN_L REAL BRACKET_L num_range(nr) BRACKET_R PAREN_R.
	{
...
constant_bnd(bnd) ::= constant_dcl_lst(names) DBL_COLON REAL BRACKET_L num_range(nr) BRACKET_R.
	{
...
attrib_spec(attr) ::= ATTRIBUTE(kw) PAREN_L sort(s) PAREN_R.
	...
	attrib_spec(attr) ::= ATTRIBUTE(kw) PAREN_L REAL BRACKET_L num_range(nr) BRACKET_R PAREN_R.
		{
			attr = NULL;
			Term const* min = nr->min();
			ObjectSymbol const* minos;
...
variable_bnd(bnd) ::= variable_lst(vars).
	{
...
/*************************************************************************************************/
/********************************************************************************************************************************/
%type      	alwayst_stmt				{ ForallTStatement*									}			// F if G...
%destructor alwayst_stmt				{ DEALLOC($$);									}

alwayst_stmt(stmt) ::= ALWAYST formula(f) IF MODE EQ INTEGER(i) PERIOD.  { stmt = new ForallTStatement(f,i->str()); }
